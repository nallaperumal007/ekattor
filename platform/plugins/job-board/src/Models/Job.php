...
class Job extends BaseModel
{
    ...
    protected function salaryText(): Attribute
    {
        return Attribute::make(
            get: function ($_, array $attributes = []) {
                ...
                if ($from || $to) {
                    if ($from && $to) {
                        return __(':from - :to /:range', ['from' => format_price($from, $this->currency), 'to' => format_price($to, $this->currency), 'range' => $salaryRange]);
                    }

                    if ($from) {
                        return __('From :price /:range', ['price' => format_price($from, $this->currency), 'range' => $salaryRange]);
                    }

                    return __('Upto :price /:range', ['price' => format_price($to, $this->currency), 'range' => $salaryRange]);
                }
    ...
    public function isJobOpen(): bool
    {
        ...
        return $this->application_closing_date->greaterThan(Carbon::now());
    }
